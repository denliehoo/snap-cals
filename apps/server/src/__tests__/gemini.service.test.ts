...
describe("gemini.service", () => {
  it("sends the description and returns parsed nutrition data", async () => {
    ...
    const result = await estimateNutrition("grilled chicken breast 200g", client);

    expect(client.models.generateContent).toHaveBeenCalledWith(
      expect.objectContaining({
        model: "gemini-2.5-flash",
        contents: "grilled chicken breast 200g",
  ...
  it("throws on empty response", async () => {
    ...
    await expect(estimateNutrition("pizza", client)).rejects.toThrow("Empty response from Gemini API");
  });

  it("throws on invalid JSON shape", async () => {
    const client = mockClient(JSON.stringify({ name: "Pizza" }));
    await expect(estimateNutrition("pizza", client)).rejects.toThrow();
  });
