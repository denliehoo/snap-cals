...
describe("useImagePicker", () => {
  ...
  it("picks from camera on granted permission", async () => {
    ...
    (ImagePicker.launchCameraAsync as jest.Mock).mockResolvedValue({ canceled: false, assets: [mockAsset] });

    const { result } = renderHook(() => useImagePicker());
    await act(() => result.current.pickFromCamera());

    expect(result.current.image).toEqual({ uri: "file://photo.jpg", base64: "abc123", mimeType: "image/jpeg" });
  });
